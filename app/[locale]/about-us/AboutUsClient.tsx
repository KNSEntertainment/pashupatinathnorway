...
export default function AboutUsClient() {
	...
	const calculateMonthsActive = () => {
		...
		const monthsDiff = (currentDate.getFullYear() - establishmentDate.getFullYear()) * 12 + (currentDate.getMonth() - establishmentDate.getMonth());
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const [aboutUsResponse, missionVisionResponse, valuesResponse, membersResponse, settingsResponse] = await Promise.all([fetch(`/api/about-us?locale=${locale}`), fetch(`/api/mission-vision?locale=${locale}`), fetch(`/api/values?locale=${locale}`), fetch(`/api/members/count`), fetch(`/api/settings`)]);
				...
				<div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
					...
					<div className="lg:col-span-5 relative">
						...
						<div className="relative rounded-2xl overflow-hidden aspect-[4/5] bg-gray-100 shadow-lg">
							<Image src={aboutUsData?.image || "/pashupatinath.png"} alt="Event Experience" width={600} height={750} className="w-full h-full object-cover" />
				...
				<div className="grid md:grid-cols-2 gap-8 lg:gap-12">
					...
					<div className="group relative bg-yellow-100 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100">
						...
						<div className="relative p-8 lg:p-10">
							...
							<h3 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-6 leading-tight">{missionVisionData?.mission?.title || t("mission_title")}</h3>
							<p className="text-gray-600 leading-relaxed text-lg">{missionVisionData?.mission?.description || t("mission_description")}</p>
						</div>
					</div>

					{/* Vision Card */}
					<div className="group relative bg-brand_secondary rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden">
						...
						<div className="relative p-8 lg:p-10 text-white">
							...
							<h3 className="text-2xl lg:text-3xl font-bold text-white mb-6 leading-tight">{missionVisionData?.vision?.title || t("vision_title")}</h3>
							<p className="text-white/90 leading-relaxed text-lg">{missionVisionData?.vision?.description || t("vision_description")}</p>
						</div>
					</div>
				</div>
			</section>
