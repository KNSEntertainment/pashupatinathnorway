...
export default function MemberEventsPage() {
	...
	const fetchEventData = useCallback(async () => {
		...
		try {
			...
			const response = await fetch(`/api/member-events?email=${encodeURIComponent(session.user.email)}`);
	...
	useEffect(() => {
		...
		fetchEventData();
	}, [sessionStatus, session?.user?.email, fetchEventData]);
			...
			<div className="max-w-7xl mx-auto">
				...
				{activeEvents.length === 0 && pastEvents.length === 0 && upcomingEvents.length === 0 && (
					<div className="text-center py-16">
						...
						<p className="text-sm text-gray-600 max-w-md mx-auto mb-8">You haven&apos;t registered for any events yet, and there are no upcoming events available.</p>
