		...
		<div className="container mx-auto">
			...
			>
				...
				<div className="md:hidden p-4">
					<button onClick={() => setProfileOpen(!profileOpen)} className="p-2 rounded hover:bg-gray-100 transition-colors">
						{profileOpen ? <X className="w-6 h-6 text-gray-800" /> : <PanelLeft className="w-6 h-6 text-gray-800" />}
					</button>
				</div>
				...
				{profileOpen && (
					<div className="md:hidden fixed inset-0 z-50 flex">
						...
						<div id="member-profile-menu" className="relative bg-brand_secondary text-white w-64 h-full shadow-lg">
							<div className="flex items-center justify-between p-4 border-b border-gray-600">
								<h2 className="text-lg font-semibold text-white">Menu</h2>
								<button onClick={() => setProfileOpen(false)} className="p-2 rounded-lg hover:bg-gray-700 transition-colors">
									...
									.map((item) => {
										...
										return (
											...
											>
												...
												{isMessages && !messageCounts.loading && <div className="flex items-center space-x-1">{messageCounts.unread > 0 && <span className="bg-red-500 text-white text-xs font-medium px-2 py-1 rounded-full min-w-[20px] text-center">{messageCounts.unread}</span>}</div>}
											</Link>
										);
									})}
							</nav>
