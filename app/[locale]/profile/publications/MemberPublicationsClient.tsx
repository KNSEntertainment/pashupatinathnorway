...
export default function MemberPublicationsClient() {
	...
	useEffect(() => {
		...
		if (searchTerm) {
			filtered = filtered.filter((publication: Publication) => publication.title.toLowerCase().includes(searchTerm.toLowerCase()) || publication.description.toLowerCase().includes(searchTerm.toLowerCase()));
		}
				...
				<div className="container mx-auto px-4 md:px-0 py-8">
					<div className="flex flex-col lg:flex-row gap-4">
						...
						<div className="flex max-w-lg relative">
							...
							<input type="text" placeholder="Search publications by title or description..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
						</div>
			...
			<div className="container mx-auto px-4 py-8">
				{loading ? (
					...
					<div className="text-center py-12">
						...
						<p className="text-gray-500">{publications.length === 0 ? "No publications are available for your membership level." : "Try adjusting your search criteria or filters."}</p>
					</div>
				) : (
					<>
								...
								{filteredPublications.map((publication: Publication) => (
									<div key={publication.id} className="p-4 hover:bg-red-50 transition-colors">
										<div className="space-y-3">
											<div>
												<h3 className="font-medium text-gray-900 md:line-clamp-2 mb-1">{publication.title}</h3>
												<div className="text-sm text-gray-500">
													{publication.description && (
														<>
															{getWordCount(publication.description) > 20 && !expandedDescriptions.has(publication.id) ? (
																<>
																	<span className="line-clamp-2">{publication.description.split(" ").slice(0, 20).join(" ")}...</span>
																	<button
																		onClick={(e) => {
																			e.stopPropagation();
																...
																<>
																	<span className={expandedDescriptions.has(publication.id) ? "" : "md:line-clamp-2"}>{publication.description}</span>
																	{getWordCount(publication.description) > 20 && expandedDescriptions.has(publication.id) && (
																		<button
																			onClick={(e) => {
																				e.stopPropagation();
											...
											<div className="flex items-center justify-between">
												...
												<button onClick={() => window.open(publication.downloadUrl, "_blank")} className="flex items-center gap-1 px-3 py-1.5 bg-red-600 text-white rounded-lg hover:bg-brand_secondary transition-colors text-xs">
								...
								<table className="w-full">
									<thead className="bg-gray-50 border-b border-gray-200">
										<tr>
											...
											<th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Published Date</th>
										...
										{filteredPublications.map((publication: Publication, index) => (
											<tr key={publication.id} className={`${index % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-red-50 transition-colors`}>
												<td className="px-6 py-4 md:max-w-sm">
													<div>
														<div className="text-sm font-medium text-gray-900 line-clamp-2 mb-1">{publication.title}</div>
														<div className="text-sm text-gray-500 line-clamp-2">{publication.description}</div>
													</div>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
													<span className={`inline-block px-3 py-1 text-xs font-medium rounded-full border ${getTypeColor(publication.type)}`}>{getTypeLabel(publication.type)}</span>
												</td>
												<td className="px-6 py-4 whitespace-nowrap">
													<div className="flex items-center">
														<span className="text-2xl mr-2">{getLanguageFlag(publication.language)}</span>
														<span className="text-sm text-gray-600">{publication.language === "en" ? "English" : publication.language === "ne" ? "नेपाली" : publication.language === "no" ? "Norsk" : "Unknown"}</span>
													</div>
												</td>
												...
												<td className="px-6 py-4 whitespace-nowrap text-sm">
													<button onClick={() => window.open(publication.downloadUrl, "_blank")} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-brand_secondary transition-colors text-sm">
