...
export default function MembershipsPage() {
	...
	return (
		<DashboardPageLayout title="Members" description="Manage temple membership applications and accounts" icon="Users">
			{/* Tab Navigation */}
			<div className="mb-8">
				<div className="border-b border-gray-200">
					<nav className="-mb-px flex space-x-8">
						<button onClick={() => setActiveTab("members")} className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === "members" ? "border-brand_primary text-brand_secondary" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"}`}>
							<div className="flex items-center gap-2">
								<Users className="w-4 h-4" />
								All Members
							</div>
						</button>
						<button onClick={() => setActiveTab("executive")} className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === "executive" ? "border-brand_primary text-brand_secondary" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"}`}>
							<div className="flex items-center gap-2">
										...
										<AlertDialogHeader>
											...
											<AlertDialogDescription>
												<div className="space-y-3">
													...
													<p className="text-sm text-gray-600">Do you want to proceed with the export?</p>
												</div>
											</AlertDialogDescription>
										</AlertDialogHeader>
										<AlertDialogFooter>
											<AlertDialogCancel>Cancel</AlertDialogCancel>
											<AlertDialogAction onClick={downloadExcel}>Download Excel</AlertDialogAction>
										</AlertDialogFooter>
									</AlertDialogContent>
								</AlertDialog>
								<AlertDialog>
									...
									<AlertDialogContent>
										...
										<AlertDialogFooter>
											<AlertDialogCancel>Cancel</AlertDialogCancel>
											<AlertDialogAction onClick={downloadCSV}>Download CSV</AlertDialogAction>
										</AlertDialogFooter>
									</AlertDialogContent>
								</AlertDialog>
					...
					<div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
						<div className="flex flex-col lg:flex-row gap-4">
							<div className="flex-1">
								<input type="text" placeholder="Search members by name, email, phone..." className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" value={search} onChange={(e) => setSearch(e.target.value)} />
							</div>
						...
						<div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
							<div className="flex flex-col sm:flex-row items-center justify-between gap-4">
								...
								<div className="flex flex-wrap gap-2">
									...
									<select className="px-3 py-1 text-sm border border-gray-300 rounded" value={bulkStatus} onChange={(e) => setBulkStatus(e.target.value)}>
									...
									<select className="px-3 py-1 text-sm border border-gray-300 rounded" value={bulkType} onChange={(e) => setBulkType(e.target.value)}>
									...
									{(() => {
										...
										return (
											<Button size="sm" variant="outline" onClick={handlePasswordReset} disabled={!hasActiveMemberSelected}>
												Reset Password
											</Button>
										);
									})()}
									...
									<div className="flex items-center gap-1">
										{getPaginationPages().map((page, index) =>
											page === "..." ? (
												...
												<Button key={page} variant={currentPage === page ? "default" : "outline"} size="sm" onClick={() => handlePageChange(page as number)} className="w-8 h-8 p-0">
													{page}
												</Button>
											),
										)}
							...
							<TableHeader>
								<TableRow className="border-b border-gray-100 bg-gray-50/50">
									<TableHead className="w-12 font-medium text-gray-700 py-4">
										<input type="checkbox" checked={allSelectedOnPage} onChange={handleSelectAll} className="w-4 h-4 rounded border-gray-300" />
									</TableHead>
							...
							<TableBody>
								{paginatedMemberships.length > 0 ? (
									paginatedMemberships.map((member: Membership) => (
										<TableRow key={member._id} className={`border-b border-gray-100 hover:bg-gray-50/50 transition-colors ${selectedMemberIds.includes(member._id) ? "bg-blue-50" : ""}`} onClick={() => handleSelectMember(member._id)}>
											<TableCell onClick={(e) => e.stopPropagation()} className="py-4">
												<input type="checkbox" checked={selectedMemberIds.includes(member._id)} onChange={() => handleSelectMember(member._id)} className="w-4 h-4 rounded border-gray-300" />
											</TableCell>
											<TableCell className="py-4">
												<div>
													<div className="font-medium text-gray-900">{`${member.firstName} ${member.middleName ? member.middleName + " " : ""}${member.lastName}`}</div>
												</div>
											</TableCell>
											<TableCell className="py-4">
											...
											<TableCell className="py-4">
												<Badge variant="outline" className={`capitalize ${member.membershipType === "General" ? "bg-blue-50 text-blue-700 border-blue-200" : member.membershipType === "Active" ? "bg-green-50 text-green-700 border-green-200" : member.membershipType === "Executive" ? "bg-purple-50 text-purple-700 border-purple-200" : member.membershipType === "Advisor" ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-gray-50 text-gray-700 border-gray-200"}`}>
													{member.membershipType}
												</Badge>
											</TableCell>
											<TableCell className="py-4">{getStatusBadge(member.membershipStatus)}</TableCell>
													...
													{member.membershipStatus === "pending" && (
														<>
															{(() => {
																...
																return (
																	<Button variant="ghost" size="sm" onClick={() => handleStatusUpdate(member._id, "approved")} disabled={!canApprove} className="h-8 px-3 text-green-600 hover:text-green-700" title={!canApprove ? "Member must be at least 15 years old" : "Approve membership"}>
																		<CheckCircle className="w-4 h-4" />
																	</Button>
																);
															})()}
															<Button variant="ghost" size="sm" onClick={() => handleStatusUpdate(member._id, "blocked")} className="h-8 px-3 text-red-600 hover:text-brand_secondary" title="Block membership">
										...
										<div>
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												<div>
													...
													<Badge variant="outline" className={`capitalize ${viewingMember.membershipType === "General" ? "bg-blue-50 text-blue-700 border-blue-200" : viewingMember.membershipType === "Active" ? "bg-green-50 text-green-700 border-green-200" : viewingMember.membershipType === "Executive" ? "bg-purple-50 text-purple-700 border-purple-200" : viewingMember.membershipType === "Advisor" ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-gray-50 text-gray-700 border-gray-200"}`}>
												...
												<div>
													...
													<p className="font-medium">{viewingMember.updatedAt ? formatDateTime(viewingMember.updatedAt) : "Not available"}</p>
												</div>
											</div>
										</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										...
										<div className="space-y-4">
											...
											<div>
												...
												<input type="text" value={editFormData.firstName || ""} onChange={(e) => handleEditChange("firstName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Middle Name</label>
												<input type="text" value={editFormData.middleName || ""} onChange={(e) => handleEditChange("middleName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Last Name *</label>
												<input type="text" value={editFormData.lastName || ""} onChange={(e) => handleEditChange("lastName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Email *</label>
												<input type="email" value={editFormData.email || ""} onChange={(e) => handleEditChange("email", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Phone *</label>
												<input type="tel" value={editFormData.phone || ""} onChange={(e) => handleEditChange("phone", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Personal Number</label>
												<div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-600">{maskPersonalNumber(editingMember?.personalNumber || "Not provided")}</div>
										...
										<div className="space-y-4">
											...
											<div>
												...
												<input type="text" value={editFormData.address || ""} onChange={(e) => handleEditChange("address", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">City</label>
												<input type="text" value={editFormData.city || ""} onChange={(e) => handleEditChange("city", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Postal Code</label>
												<input type="text" value={editFormData.postalCode || ""} onChange={(e) => handleEditChange("postalCode", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>
											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Fylke</label>
												<input type="text" value={editFormData.fylke || ""} onChange={(e) => handleEditChange("fylke", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>
											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Kommune</label>
												<input type="text" value={editFormData.kommune || ""} onChange={(e) => handleEditChange("kommune", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
											</div>
										</div>
										...
										<div className="space-y-4">
											...
											<div>
												...
												{(() => {
													...
													return (
														<>
															<select value={editFormData.membershipType || "General"} onChange={(e) => handleEditChange("membershipType", e.target.value)} className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:border-transparent ${isUnder15 ? "border-gray-300 bg-gray-100 text-gray-500 cursor-not-allowed" : "border-gray-300 focus:ring-blue-500 focus:border-transparent"}`} disabled={isUnder15} required>
																<option value="General">General Member</option>
																<option value="Active">Active Member</option>
																<option value="Executive">Executive Member</option>
																<option value="Advisor">Advisor</option>
															</select>
															{isUnder15 && <p className="text-sm text-orange-600 mt-1">Membership type cannot be changed for members under 15 years old</p>}
														</>
													);
												})()}
											</div>
											...
											{["Executive", "Advisor"].includes(editFormData.membershipType || "") && (
												<div>
													...
													<input type="text" value={editFormData.position || ""} onChange={(e) => handleEditChange("position", e.target.value)} placeholder="e.g., Chairperson, Secretary, Advisor" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
												</div>
											)}

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Membership Status *</label>
												<select value={editFormData.membershipStatus || "pending"} onChange={(e) => handleEditChange("membershipStatus", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
													<option value="pending">Pending</option>
													<option value="approved">Approved</option>
													<option value="blocked">Blocked</option>
												</select>
								...
								<form onSubmit={handleAddSubmit} className="overflow-y-auto max-h-[calc(90vh-180px)] px-8 py-6">
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										...
										<div className="space-y-4">
											...
											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">First Name *</label>
												<input type="text" value={addFormData.firstName || ""} onChange={(e) => handleAddChange("firstName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Middle Name</label>
												<input type="text" value={addFormData.middleName || ""} onChange={(e) => handleAddChange("middleName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Last Name *</label>
												<input type="text" value={addFormData.lastName || ""} onChange={(e) => handleAddChange("lastName", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Email *</label>
												<input type="email" value={addFormData.email || ""} onChange={(e) => handleAddChange("email", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Phone *</label>
												<input type="tel" value={addFormData.phone || ""} onChange={(e) => handleAddChange("phone", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Personal Number</label>
												<input type="text" value={addFormData.personalNumber || ""} onChange={(e) => handleAddChange("personalNumber", e.target.value)} placeholder="11-digit personal number" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Gender</label>
												<select value={addFormData.gender || ""} onChange={(e) => handleAddChange("gender", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
										...
										<div className="space-y-4">
											...
											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Address</label>
												<input type="text" value={addFormData.address || ""} onChange={(e) => handleAddChange("address", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">City</label>
												<input type="text" value={addFormData.city || ""} onChange={(e) => handleAddChange("city", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Postal Code</label>
												<input type="text" value={addFormData.postalCode || ""} onChange={(e) => handleAddChange("postalCode", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Fylke</label>
												<input type="text" value={addFormData.fylke || ""} onChange={(e) => handleAddChange("fylke", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Kommune</label>
												<input type="text" value={addFormData.kommune || ""} onChange={(e) => handleAddChange("kommune", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" />
											</div>

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Membership Type *</label>
												<select value={addFormData.membershipType || "General"} onChange={(e) => handleAddChange("membershipType", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required>
													<option value="General">General Member</option>
													<option value="Active">Active Member</option>
													<option value="Executive">Executive Member</option>
													<option value="Advisor">Advisor</option>
											...
											{["Executive", "Advisor"].includes(addFormData.membershipType || "") && (
												<div>
													<label className="block text-sm font-medium text-gray-900 mb-2">Position *</label>
													<input type="text" value={addFormData.position || ""} onChange={(e) => handleAddChange("position", e.target.value)} placeholder="e.g., Chairperson, Secretary, Advisor" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required />
												</div>
											)}

											<div>
												<label className="block text-sm font-medium text-gray-900 mb-2">Membership Status *</label>
												<select value={addFormData.membershipStatus || "pending"} onChange={(e) => handleAddChange("membershipStatus", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" required>
													<option value="pending">Pending</option>
													<option value="approved">Approved</option>
													<option value="blocked">Blocked</option>
												</select>
