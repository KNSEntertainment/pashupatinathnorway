...
export default function PublicationsPage() {
	...
	useEffect(() => {
		...
		if (searchTerm) {
			filtered = filtered.filter((pub) => pub.title.toLowerCase().includes(searchTerm.toLowerCase()) || pub.description.toLowerCase().includes(searchTerm.toLowerCase()));
		}
	...
	if (showReportTypesManagement) {
		return <ReportTypesManagement onSubmit={handleReportTypesSubmit} onCancel={() => setShowReportTypesManagement(false)} />;
	}
		...
		<DashboardPageLayout
			...
			actions={
				<div className="flex items-center gap-4">
					<button onClick={() => setShowReportTypesManagement(true)} className="flex items-center gap-2 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
				...
				<div className="container mx-auto px-4 py-4">
					<div className="flex flex-col lg:flex-row gap-4">
						...
						<div className="flex-1 relative">
							...
							<input type="text" placeholder="Search publications..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
						</div>
						...
						<button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
							...
							Filters
							<span className="text-xs bg-gray-200 px-2 py-1 rounded-full">{[selectedType, selectedLanguage].filter((v) => v !== "all").length}</span>
						</button>
					</div>
			...
			<div className="container mx-auto px-4 py-6">
				{loading ? (
					...
					<div className="text-center py-12">
						...
						<p className="text-gray-500 mb-4">{publications.length === 0 ? "Start by adding your first publication." : "Try adjusting your search criteria or filters."}</p>
							...
							<div key={publication.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200">
								<div className="p-6">
									...
									<div className="flex gap-2">
										...
										<button onClick={() => handleDelete(publication.id)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 border border-red-300 text-brand_secondary rounded-lg hover:bg-red-50 transition-colors text-sm">
