...
Anonymous User,anonymous@temple.no,,250,NOK,Anonymous donation,,true,completed,`;
	...
	return (
		<DashboardPageLayout
			...
			description="Upload multiple donation records at once using a CSV file. Follow the template format for best results."
			icon="Heart"
			breadcrumbs={
		...
		>
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<Card>
					...
					<CardContent className="space-y-4">
						<div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${dragActive ? "border-blue-500 bg-blue-50" : "border-gray-300"}`} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave}>
							<Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
							<p className="text-gray-600 mb-4">Drag and drop your CSV file here, or click to browse</p>
							<input id="file-input" type="file" accept=".csv" onChange={(e) => e.target.files && handleFileSelect(e.target.files[0])} className="hidden" />
						...
						<div className="flex gap-4">
							<AlertDialog>
								<AlertDialogTrigger asChild>
									<Button disabled={!file || uploading} className="flex-1">
										{uploading ? (
											<>
								...
								<AlertDialogContent>
									<AlertDialogHeader>
										...
										<AlertDialogDescription>
											<div className="space-y-3">
												<p>
													You are about to upload donations from <strong>{file?.name}</strong>({file ? (file.size / 1024).toFixed(1) : 0} KB).
												</p>
												<div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
													<p className="text-sm text-orange-800">
														<strong>Important:</strong> This action will be recorded with your user information and timestamp. The system will log who performed this bulk upload and when it occurred.
													</p>
												</div>
									...
									<AlertDialogFooter>
										...
										<AlertDialogAction onClick={handleUpload}>Yes, Upload Donations</AlertDialogAction>
									</AlertDialogFooter>
								</AlertDialogContent>
							</AlertDialog>
			...
			{/* Upload Results */}
			{uploadResult && (
				<Card className={`mt-8 ${uploadResult.success ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
					<CardHeader>
						<CardTitle className={`flex items-center gap-2 ${uploadResult.success ? "text-green-900" : "text-brand_secondary"}`}>
					...
					<CardContent>
						{uploadResult.success ? (
							...
							<div className="space-y-2">
								<p className="font-medium text-brand_secondary">{uploadResult.error}</p>
								{uploadResult.validationErrors && uploadResult.validationErrors.length > 0 && (
									<div className="mt-4">
										<h4 className="font-semibold text-brand_secondary mb-2">Validation Errors:</h4>
