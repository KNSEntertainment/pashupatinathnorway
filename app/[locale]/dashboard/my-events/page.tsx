...
export default function MemberEventsPage() {
	...
	useEffect(() => {
		...
		const getUserData = () => {
			// Try to get from session first (better approach)
	...
	const fetchEventData = async (email: string) => {
		try {
			setLoading(true);

			// Fetch member events data using the optimized API
			const response = await fetch(`/api/member-events?email=${encodeURIComponent(email)}`);
			const data = await response.json();

			if (!data.success) {
				throw new Error(data.error || "Failed to fetch member events");
			}

			const { activeEvents, pastEvents, upcomingEvents } = data.data;
	...
	return (
		<DashboardPageLayout title="My Events" description="Manage your event tickets and discover upcoming events" icon="Calendar">
								...
								<CardContent className="p-6">
									...
									<div className="mt-6 text-center">
										<div className="inline-block p-4 bg-white rounded-lg shadow-sm border">
											<Image src={generateQRCode(registration.registrationId)} alt="Event QR Code" width={128} height={128} />
						...
						{upcomingEvents.map((event) => (
							<Card key={event._id} className="overflow-hidden hover:shadow-lg transition-shadow">
								...
								<CardContent className="p-6">
									...
									{event.eventdescription && <p className="text-gray-600 text-sm mb-4 line-clamp-2">{event.eventdescription}</p>}

									<Link href={`/register?eventId=${event._id}`} className="inline-flex items-center justify-center w-full gap-2 bg-purple-600 text-white hover:bg-purple-700 px-4 py-2 rounded-lg font-medium text-sm transition-colors">
			...
			{activeEvents.length === 0 && pastEvents.length === 0 && upcomingEvents.length === 0 && (
				<div className="text-center py-16">
					...
					<p className="text-gray-600 max-w-md mx-auto mb-8">You haven&apos;t registered for any events yet, and there are no upcoming events available.</p>
					<Link href="/events" className="inline-flex items-center gap-2 bg-brand_secondary text-white hover:bg-red-800 px-6 py-3 rounded-lg font-medium transition-colors">
						Browse All Events
					</Link>
				</div>
			)}
