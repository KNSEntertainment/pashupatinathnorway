			...
			<div className="container mx-auto md:px-4 md:pb-12">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
					...
					<div className="md:col-span-2 md:mt-8 md:shadow-md p-8 md:px-12 bg-brand_primary relative overflow-hidden">
						...
						<div className="absolute top-4 right-4 z-20 mb-4 md:mb-0 text-right">
						...
						<form className="relative z-10 space-y-6" onSubmit={handleSubmit} noValidate>
							...
							<section>
								...
								<div className="grid md:grid-cols-2 gap-6">
									{/* Personal Number */}
									<FormField label={t.personal_number} required error={form.errors.personalNumber}>
									...
									<FormField label={t.first_name} required error={form.errors.firstName}>
										<StyledInput type="text" name="firstName" value={form.formData.firstName} onChange={form.handleChange} hasError={!!form.errors.firstName} placeholder={t.first_name_placeholder} />
									</FormField>

									{/* Middle Name */}
									<FormField label={t.middle_name}>
										<StyledInput type="text" name="middleName" value={form.formData.middleName} onChange={form.handleChange} placeholder={t.middle_name_placeholder} />
									</FormField>

									{/* Last Name */}
									<FormField label={t.last_name} required error={form.errors.lastName}>
										<StyledInput type="text" name="lastName" value={form.formData.lastName} onChange={form.handleChange} hasError={!!form.errors.lastName} placeholder={t.last_name_placeholder} />
									</FormField>

									{/* Email */}
									<FormField label={t.email_address} required error={form.errors.email}>
										<StyledInput type="email" name="email" value={form.formData.email} onChange={form.handleChange} onBlur={form.handleEmailBlur} hasError={!!form.errors.email} placeholder={t.email_address_placeholder} />
									</FormField>

									{/* Phone */}
									<FormField label={t.phone_number} required error={form.errors.phone}>
										<div className="relative">
											<StyledInput type="tel" maxLength={8} name="phone" value={form.formData.phone} onChange={form.handleChange} onBlur={form.handlePhoneBlur} readOnly={otp.phoneVerified} hasError={!!form.errors.phone} isSuccess={otp.phoneVerified} className={otp.phoneVerified ? "bg-gray-50 cursor-not-allowed" : ""} placeholder={t.phone_number_placeholder} />
											{otp.phoneVerified && (
										...
										{otp.phoneVerified && <p className="text-green-600 text-sm mt-1">Phone number verified</p>}
									</FormField>
								</div>
							</section>
							...
							<section>
								<div className="hidden md:flex justify-between items-center mb-4">
									...
									<button type="button" onClick={form.addFamilyMember} disabled={!form.isMainApplicantOver15() || !!form.errors.personalNumber} className={`bg-brand_secondary text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium ${!form.isMainApplicantOver15() || !!form.errors.personalNumber ? "opacity-50 cursor-not-allowed" : "hover:bg-rd-700"}`}>
										{tr("add_children_u15")}
									</button>
								</div>
								...
								{form.formData.familyMembers.length === 0 ? (
									...
									<div className="space-y-4">
										{form.formData.familyMembers.map((member, index) => (
											<FamilyMemberCard key={member.id} member={member} index={index} errors={form.familyMemberErrors} onUpdate={form.updateFamilyMember} onRemove={form.removeFamilyMember} labels={familyMemberLabels} />
										))}
									</div>
								)}
							</section>
							...
							<div className="rounded-lg p-2 md:p-6">
								<label className="flex items-start cursor-pointer">
									<input type="checkbox" name="agreeTerms" checked={form.formData.agreeTerms} onChange={form.handleChange} className="w-5 h-5 text-brand_primary rounded mt-1" />
								...
								{form.errors.terms && <p className="text-red-600 text-sm mt-2">{form.errors.terms}</p>}
							</div>

							{/* ── Buttons ── */}
							<div className="flex gap-4">
								<button type="submit" disabled={!form.isFormValid()} className={`flex-1 bg-brand_secondary text-white py-2 md:py-4 px-6 md:px-8 rounded-lg font-semibold transition-colors shadow-lg hover:shadow-xl${!form.isFormValid() ? " opacity-50 cursor-not-allowed" : ""}`}>
									{t.submit}
								</button>
								<button
									type="button"
						...
						<Card className="bg-brand_primary/10 md:backdrop-blur-sm">
							...
							<CardContent className="p-6 space-y-4">
								...
								<div className="space-y-3">
									...
									<div className="flex items-start gap-3">
										<div className="w-2 h-2 rounded-full bg-brand_primary mt-2 flex-shrink-0" />
										<p className="min-w-0 break-words text-gray-700 leading-relaxed text-sm">
											<a href="https://person.brreg.no/en/minside/tros-og-livssyn" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline break-all" aria-label="View existing memberships with other organizations on Brreg website (opens in new tab)">
												Click to View
											</a>{" "}
											<span className="break-all">(https://person.brreg.no/en/minside)</span> your existing memberships with other organizations. To cancel,{" "}
											<a href="/utmeldingsblankett-juli2006.pdf" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline" aria-label="Download membership cancellation form (PDF, opens in new tab)">
												fill out this form
											</a>{" "}
											and send it to the respective temple/organization.{" "}
										</p>
									</div>
									<div className="flex items-start gap-3">
										<div className="w-2 h-2 rounded-full bg-brand_primary mt-2 flex-shrink-0" />
