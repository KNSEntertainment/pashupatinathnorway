...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (search) {
			query.$or = [{ firstName: { $regex: search, $options: "i" } }, { lastName: { $regex: search, $options: "i" } }, { email: { $regex: search, $options: "i" } }, { phone: { $regex: search, $options: "i" } }];
		}

		const memberships = await Membership.find(query).sort({ createdAt: -1 });

		// Prepare data for Excel
		const excelData = memberships.map((member) => {
		...
		return NextResponse.json({ error: "Internal server error during Excel download" }, { status: 500 });
	}
}
