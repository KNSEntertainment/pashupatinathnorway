...
const colorOptions = [
	...
	{ value: "yellow", label: "Yellow", className: "bg-yellow-100 text-brand_primary border-yellow-200" },
...
function ReportTypeForm({ reportType, onSubmit, onCancel }: ReportTypeFormProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const url = reportType?.id ? `/api/report-types/${reportType.id}` : "/api/report-types";
			...
			<div className="bg-white border-b">
				<div className="container mx-auto px-4 py-6">
					<div className="flex items-center justify-between">
						<div>
							<h1 className="text-2xl font-bold text-gray-900">{reportType ? "Edit Report Type" : "Add New Report Type"}</h1>
							<p className="text-gray-600 mt-1">{reportType ? "Update report type details" : "Create a new report type"}</p>
						</div>
					...
					<form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
						<div className="grid grid-cols-1 md:grid-cols-3 items-end gap-6">
							...
							<div>
								...
								<input type="text" name="label" value={formData.label} onChange={handleChange} className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 ${errors.label ? "border-red-500" : "border-gray-300"}`} placeholder="e.g., Financial Reports" />
							...
							<div>
								...
								<select name="color" value={formData.color} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
						...
						<div className="flex space-x-2 mt-6 p-4 bg-gray-50 rounded-lg">
							...
							<span className={`px-3 py-1 text-xs font-medium rounded-full border ${colorOptions.find((c) => c.value === formData.color)?.className || "bg-gray-100 text-gray-800 border-gray-200"}`}>{formData.label || "Report Type Label"}</span>
						</div>
						...
						<div className="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
							...
							<button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-brand_secondary transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
								{loading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <Plus className="w-4 h-4" />}
...
export default function ReportTypesManagement({ onSubmit, onCancel }: ReportTypesManagementProps = {}) {
	...
	useEffect(() => {
		...
		if (searchTerm) {
			filtered = filtered.filter((type) => type.name.toLowerCase().includes(searchTerm.toLowerCase()) || type.label.toLowerCase().includes(searchTerm.toLowerCase()) || type.description.toLowerCase().includes(searchTerm.toLowerCase()));
		}

		if (filterStatus !== "all") {
			filtered = filtered.filter((type) => (filterStatus === "active" ? type.isActive : !type.isActive));
		}
			...
			<div className="bg-white border-b">
				<div className="container mx-auto px-4 py-6">
					<div className="flex items-center justify-between">
						...
						<button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-brand_secondary transition-colors">
				...
				<div className="container mx-auto px-4 py-4">
					<div className="flex flex-col lg:flex-row gap-4">
						...
						<div className="flex-1 relative">
							...
							<input type="text" placeholder="Search report types..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
						</div>
					...
					{showFilters && (
						<div className="mt-4">
							<select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
			...
			<div className="container mx-auto px-4 py-6">
				{loading ? (
					...
					<div className="text-center py-12">
						...
						<p className="text-gray-500 mb-4">{reportTypes.length === 0 ? "Start by adding your first report type." : "Try adjusting your search criteria or filters."}</p>
						{reportTypes.length === 0 && (
							<button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-brand_secondary transition-colors">
								<Plus className="w-4 h-4" />
									...
									<tr key={reportType.id} className="hover:bg-gray-50">
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="flex items-center">
												<span className={`inline-block px-3 py-1 text-xs font-medium rounded-full border ${getColorClass(reportType.color)}`}>{reportType.label}</span>
											</div>
										</td>
										<td className="px-6 py-4 whitespace-nowrap">{reportType.isActive ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Active</span> : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Inactive</span>}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm">
											<div className="flex gap-2">
												...
												<button onClick={() => handleDelete(reportType.id)} className="flex items-center gap-1 px-3 py-1 border border-red-300 text-brand_secondary rounded hover:bg-red-50 transition-colors text-xs">
