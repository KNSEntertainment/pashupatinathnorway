...
import { Users, DollarSign, Calendar, TrendingUp, TrendingDown, Activity, UserCheck, CreditCard, Target } from "lucide-react";
				...
				<div className="flex-1">
					...
					{change !== undefined && (
						<div className="flex items-center mt-2">
							...
							<span className={`text-sm ${change >= 0 ? "text-green-600" : "text-red-600"}`}>{Math.abs(change).toFixed(1)}%</span>
						</div>
					)}
				</div>
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				<MetricCard title="Total Members" value={stats.overview.totalMembers.toLocaleString()} icon={UserCheck} color="text-orange-600" bgColor="bg-orange-100" />
				<MetricCard title="Total Donations" value={`NOK ${stats.financial.totalDonations.toLocaleString()}`} change={stats.financial.donationGrowth} icon={DollarSign} color="text-green-600" bgColor="bg-green-100" />
				<MetricCard title="Total Events" value={stats.overview.totalEvents.toLocaleString()} icon={Calendar} color="text-purple-600" bgColor="bg-purple-100" />
				<MetricCard title="Total Income" value={`NOK ${stats.financial.totalIncome.toLocaleString()}`} icon={TrendingUp} color="text-green-600" bgColor="bg-green-100" />
				<MetricCard title="Total Expenses" value={`NOK ${stats.financial.totalExpenses.toLocaleString()}`} icon={TrendingDown} color="text-red-600" bgColor="bg-red-100" />
				<MetricCard title="Net Balance" value={`NOK ${(stats.financial.totalIncome - stats.financial.totalExpenses).toLocaleString()}`} icon={Target} color={stats.financial.totalIncome - stats.financial.totalExpenses >= 0 ? "text-green-600" : "text-red-600"} bgColor={stats.financial.totalIncome - stats.financial.totalExpenses >= 0 ? "bg-green-100" : "bg-red-100"} />
			</div>
					...
					<CardContent>
						<ResponsiveContainer width="100%" height={300}>
							<LineChart data={stats.charts.membershipTrends}>
								...
								<Tooltip />
								<Legend />
								<Line type="monotone" dataKey="count" stroke="#8884d8" strokeWidth={2} name="New Members" dot={{ fill: "#8884d8", r: 4 }} activeDot={{ r: 6 }} />
							</LineChart>
						</ResponsiveContainer>
					</CardContent>
				</Card>
						...
						<div className="space-y-3">
							{stats.recentActivity.donations.map((donation) => (
								<div key={donation.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
									...
									<span className="font-semibold text-green-600">NOK {donation.amount.toLocaleString()}</span>
								</div>
							))}
						</div>
					</CardContent>
					...
					<CardContent>
						<div className="space-y-3">
							{stats.recentActivity.transactions.map((transaction) => (
								<div key={transaction.id} className={`p-2 rounded ${transaction.type === "income" ? "bg-green-50 border-l-4 border-green-500" : "bg-red-50 border-l-4 border-red-500"}`}>
							...
							{stats.recentActivity.users.map((user) => (
								<div key={user.id} className="p-2 bg-gray-50 rounded">
									...
									<div className="flex justify-between items-center mt-1">
										...
										<span className={`text-xs px-2 py-1 rounded ${user.role === "admin" ? "bg-red-100 text-brand_secondary" : "bg-blue-100 text-blue-700"}`}>{user.role}</span>
									</div>
								</div>
							))}
						</div>
