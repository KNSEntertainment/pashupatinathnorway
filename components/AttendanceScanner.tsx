			...
			<div className="text-center space-y-6">
				...
				{error && (
					<Alert className="border-red-200 bg-red-50">
						...
						<AlertDescription className="text-brand_secondary">{error}</AlertDescription>
					</Alert>
				)}
				...
				<div className="space-y-4">
					<Button onClick={startCameraScan} disabled={isScanning} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg h-12 text-base font-medium">
					...
					<Button onClick={() => fileInputRef.current?.click()} variant="outline" className="w-full h-12 text-base font-medium border-2 border-gray-300 hover:border-blue-500 hover:bg-blue-50">
					...
					<div className="mt-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl">
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-medium text-gray-900 text-xs">{lastScanned.attendance?.checkInTime ? new Date(lastScanned.attendance.checkInTime).toLocaleTimeString() : "N/A"}</span>
							</div>
							<div className="mt-3 pt-3 border-t border-green-200">
								<Badge className="bg-green-600 text-white hover:bg-green-700">✓ Successfully Checked In</Badge>
							</div>
						</div>
					</div>
				)}
