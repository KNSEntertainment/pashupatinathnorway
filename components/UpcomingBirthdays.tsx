...
export default function UpcomingBirthdays() {
	...
	const getDaysColor = (days: number) => {
		...
		if (days <= 3) return "bg-brand_primary text-white";
					...
					{birthdays.map((member: BirthdayMember) => (
						<div key={member._id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
							<div className="flex items-center gap-4">
								{member.profilePhoto ? (
									<Image src={member.profilePhoto} alt={member.fullName} width={48} height={48} className="w-12 h-12 rounded-full object-cover border-2 border-gray-200" />
								) : (
								...
								<div>
									...
									<div className="flex items-center gap-2 text-sm text-gray-600">
										<Badge variant="outline" className={`capitalize ${member.membershipType === "general" ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-green-50 text-green-700 border-green-200"}`}>
							...
							<div className="flex items-center gap-3">
								<Badge className={getDaysColor(member.daysUntilBirthday)}>{getDaysText(member.daysUntilBirthday)}</Badge>
								{member.daysUntilBirthday === 0 && (
									<Button size="sm" variant="outline" onClick={() => sendBirthdayEmail(member)} className="flex items-center gap-2">
