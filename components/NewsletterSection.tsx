...
export default function NewsletterSection() {
	...
	return (
		<section className="py-20 bg-brand_secondary relative overflow-hidden w-screen left-1/2 right-1/2 -translate-x-1/2">
			{/* Decorative Elements */}
				...
				<div className="max-w-6xl mx-auto">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
						...
						<div className="text-center lg:text-left">
							...
							<p className="text-xl text-white/90 mb-8 max-w-lg">{t("description")}</p>

							<p suppressHydrationWarning className="text-white/70 text-sm">
								{t("footer_text")}
							</p>
						</div>
						...
						<div>
							<form onSubmit={handleSubmit} className="space-y-4">
								<div className="relative">
									<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t("email_placeholder")} className="w-full px-6 py-4 rounded-lg bg-white/90 backdrop-blur-sm border border-white/20 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white transition-all duration-300" required disabled={isSubmitting || isSubmitted} />
									<Mail className="absolute right-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
								</div>

								<button type="submit" disabled={isSubmitting || isSubmitted} className="w-full px-8 py-4 bg-white text-gray-700 font-semibold rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2">
									{isSubmitting ? (
										<>
