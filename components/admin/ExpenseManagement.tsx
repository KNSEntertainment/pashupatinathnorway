...
import { Plus, Edit, Trash2, Save, X, RefreshCw, DollarSign, TrendingDown, Calendar, FileText, Filter, Receipt, Upload } from "lucide-react";
...
const expenseCategories = ["Salary Expenses", "Tax Expenses", "Building And Premises", "Furniture And Equipment", "Office Expenses", "Events Courses Projects", "Internal Transfers", "Transfers Within Norway", "Transfers Abroad", "Financial Expenses", "Other Expenses"];
	...
	return (
		<div className="p-6 max-w-7xl mx-auto">
			<div className="flex justify-between items-center mb-6">
				...
				<button onClick={() => setIsCreating(true)} className="flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors">
					<Plus size={20} />
					Add Expense
				</button>
			</div>
				...
				<div className="bg-white p-4 rounded-lg shadow border border-gray-200">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-2xl font-bold text-red-600">kr{expenses.reduce((sum, e) => sum + e.amount, 0).toLocaleString()}</p>
						</div>
				...
				<div className="bg-white p-4 rounded-lg shadow border border-gray-200">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-2xl font-bold text-purple-600">kr{expenses.length > 0 ? (expenses.reduce((sum, e) => sum + e.amount, 0) / expenses.length).toFixed(2) : "0"}</p>
						</div>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<select required value={formData.expenseCategory} onChange={(e) => setFormData({ ...formData, expenseCategory: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
							...
							<div>
								...
								<select value={typeof formData.budgetId === "string" ? formData.budgetId : ""} onChange={(e) => setFormData({ ...formData, budgetId: e.target.value || null })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
						...
						<div>
							...
							<div className="flex items-center gap-4">
								<input
									...
									id="receipt-upload"
								/>
								<label htmlFor="receipt-upload" className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
									<Upload size={16} />
									{uploadingReceipt ? "Uploading..." : "Choose Receipt"}
								</label>
								{formData.receiptUrl && (
									<a href={formData.receiptUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-500 hover:text-blue-700">
										<Receipt size={16} />
										View Receipt
									</a>
								)}
						...
						<div>
							...
							<textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Additional details about this expense..." />
						</div>

						<div className="flex gap-2">
							<button type="submit" className="flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors">
								<Save size={20} />
						...
						<tbody className="divide-y divide-gray-200">
							{expenses.map((expense) => (
								<tr key={expense._id} className="hover:bg-gray-50">
									...
									<td className="px-4 py-3 font-medium text-red-600">kr{expense.amount.toLocaleString()}</td>
									<td className="px-4 py-3">
										<span className="text-sm text-gray-600">{new Date(expense.date).toLocaleDateString()}</span>
									</td>
									<td className="px-4 py-3">
										{expense.receiptUrl ? (
											<a href={expense.receiptUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-500 hover:text-blue-700">
												<Receipt size={16} />
												View
											</a>
										) : (
									...
									<td className="px-4 py-3">
										<div className="flex gap-2">
											...
											<button onClick={() => handleDelete(expense._id!)} className="text-red-500 hover:text-brand_secondary">
												<Trash2 size={16} />
											</button>
										</div>
									</td>
