...
// Config must run before any export function can be called
cloudinary.v2.config({
  cloud_name: process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME,
  api_key: process.env.CLOUDINARY_API_KEY,
  api_secret: process.env.CLOUDINARY_API_SECRET,
...
export async function deleteFromCloudinary(url, resourceType) {
  ...
  if (!resourceType) {
    ...
    if (["jpg", "jpeg", "png", "gif", "webp", "bmp", "tiff", "svg"].includes(extLower)) {
      resourceType = "image";
    } else if (["mp4", "mov", "avi", "wmv", "flv", "mkv", "webm"].includes(extLower)) {
      resourceType = "video";
    } else {
      resourceType = "raw";
    }
...
export async function uploadToCloudinary(file, folder = "default_folder") {
  ...
  if (
    ...
    throw new Error(
      "Cloudinary env vars missing: NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET"
    );
  }
